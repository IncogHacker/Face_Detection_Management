...
from Student import Student #whenever we click on student it open Student section for this make a function
...
class Face_Recognition_System:
    def __init__(self, root):
        ...
        b1=Button(f_lbl,image=self.stdudentimage,command=self.student_details,cursor="hand2",bg="#a8c0ff")
        b1.place(x=200,y=200,width=220,height=220)


        student_label =Button(f_lbl, text="STUDENT DETAILS", command=self.student_details,
                              cursor="hand2",
...
# -------------------------------------------------------------------
    ...
    def student_details(self): # Function that runs when user clicks on "Student" or photo
             self.new_window=Toplevel(self.root)# Opens a new window top of your main windows (child window of the main/root window)

            # Toplevel() is a Tkinter widget used to create a new window on top of the main window (self.root).
            # Self.root is typically your main GUI window.
            # self.new_window becomes a reference to this newly created window, so you can interact with it later

             self.open=Student(self.new_window) # Creates a Student object using the new window as its root 
      
                  # This line initializes a new object of the Student class, passing the new_window as its root.
 #It means the Student class is likely another window layout or interface designed to show student details, and it's now being attached to the new pop-up window.
# When the user clicks a "Student" button or image, this function opens a new top-level window and loads the student detail interface (Student) inside that window.
